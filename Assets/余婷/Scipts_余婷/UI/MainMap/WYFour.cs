...
public class WYFour : MonoBehaviour
{
    AudioSource ani;
    private static WYFour _instance;
    public static WYFour Instance
    ...
    void Start()
    {
        SaveIndexMgr.Instance.SaveSceneId =4;
        ...
        if (SaveIndexMgr.Instance.CurindexMapFour != -1)
        {
            Zj.transform.position = AllPointList[SaveIndexMgr.Instance.CurindexMapFour].transform.position;
        }
    ...
    private void Yd(int Index)
    {
        ...
        {
            pointIndex = Index;
            SaveIndexMgr.Instance.SaveMapFour(pointIndex);
