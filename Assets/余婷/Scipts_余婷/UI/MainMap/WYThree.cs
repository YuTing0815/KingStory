...
public class WYThree : MonoBehaviour
{
    AudioSource ani;
    private static WYThree _instance;
    public static WYThree Instance
    ...
    void Start()
    {
        ...
        if (SaveIndexMgr.Instance.CurindexMapThree != -1)
        {
            Zj.transform.position = AllPointList[SaveIndexMgr.Instance.CurindexMapThree].transform.position;
        }
    ...
    private void Yd(int Index)
    {
        ...
        {
            pointIndex = Index;
            SaveIndexMgr.Instance.SaveMapThree(pointIndex);
